...
@router.post("/margin/settings")
async def save_margin_settings(
    request: Request,
    user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    if not user or user.role != UserRole.owner:
        return RedirectResponse("/reports/margin", status_code=302)
    form = await request.form()
    for dept in BillingDept:
        key = f"labor_cost.{dept.value}"
        val = str(form.get(f"rate_{dept.value}", "0")).strip()
        try:
            float(val)
        except ValueError:
            val = "0"
    ...
    db.commit()
    return RedirectResponse("/reports/margin?saved=1", status_code=302)
